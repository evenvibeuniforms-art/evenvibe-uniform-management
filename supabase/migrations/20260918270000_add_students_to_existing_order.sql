-- Migration: Add Students to Existing Order RPCs
-- Implements get_eligible_students_for_order and add_students_to_existing_order

-- 1. Function to retrieve all eligible completed students for an order
...
-- 2. Function to add selected students to existing order atomically
...
-- Secure functions
REVOKE EXECUTE ON FUNCTION public.get_eligible_students_for_order(UUID) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.get_eligible_students_for_order(UUID) TO authenticated, service_role;

REVOKE EXECUTE ON FUNCTION public.add_students_to_existing_order(UUID, UUID[]) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.add_students_to_existing_order(UUID, UUID[]) TO authenticated, service_role;
