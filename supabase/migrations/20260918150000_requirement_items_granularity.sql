-- Migration: Add class and section granularity to requirement_items

-- 1. Modify requirement_items table to add class_name and section_name
ALTER TABLE public.requirement_items
    ADD COLUMN IF NOT EXISTS class_name TEXT,
    ADD COLUMN IF NOT EXISTS section_name TEXT;

-- We don't add NOT NULL because legacy items will be NULL.
-- Drop the existing unique index to replace it with the granular one
DROP INDEX IF EXISTS public.req_items_unique_idx;

-- Create the new granular unique index safely combining NULLs using COALESCE
CREATE UNIQUE INDEX req_items_unique_idx ON public.requirement_items (
    requirement_id, 
    COALESCE(class_name, 'Legacy'),
    COALESCE(section_name, 'Legacy'),
